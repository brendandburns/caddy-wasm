...
void exports_wasi_http_incoming_handler_handle(exports_wasi_http_incoming_handler_own_incoming_request_t request, exports_wasi_http_incoming_handler_own_response_outparam_t response_out) {
    ...
    wasi_http_types_method_incoming_request_path_with_query(wasi_http_types_borrow_incoming_request(request), &path_query);
    char* path_query_cstr = c_str(&path_query);
    wasi_http_types_method_incoming_request_authority(wasi_http_types_borrow_incoming_request(request), &authority);
    ...
    wasi_http_types_method_outgoing_response_body(wasi_http_types_borrow_outgoing_response(res), &body);

    wasi_http_types_result_own_outgoing_response_error_code_t res_err;
    res_err.is_err = false;
    res_err.val.ok = res;
    wasi_http_types_static_response_outparam_set(response_out, &res_err);

    wasi_http_types_own_output_stream_t stream;
    wasi_http_types_method_outgoing_body_write(wasi_http_types_borrow_outgoing_body(body), &stream);

    client_list_u8_t contents;
    ...
    wasi_io_streams_method_output_stream_blocking_write_and_flush(wasi_io_streams_borrow_output_stream(stream), &contents, &stream_err);

    wasi_io_streams_output_stream_drop_own(stream);

    wasi_http_types_error_code_t error_code;
    wasi_http_types_static_outgoing_body_finish(body, NULL, &error_code);
...
int wasi_http_request(uint8_t method_tag, uint8_t scheme_tag, const char * authority_str, const char* path_query_str, const char* body, wasi_http_response_t* response_out) {
    ...
    wasi_http_types_header_error_t err;
    if (!wasi_http_types_static_fields_from_list(&headers_list, &headers, &err)) {
    ...
    wasi_http_types_own_outgoing_request_t req = wasi_http_types_constructor_outgoing_request(headers);
    bool ok = wasi_http_types_method_outgoing_request_set_method(wasi_http_types_borrow_outgoing_request(req), &method) &&
        wasi_http_types_method_outgoing_request_set_path_with_query(wasi_http_types_borrow_outgoing_request(req), &path_query) &&
        wasi_http_types_method_outgoing_request_set_scheme(wasi_http_types_borrow_outgoing_request(req), &scheme) &&
        wasi_http_types_method_outgoing_request_set_authority(wasi_http_types_borrow_outgoing_request(req), &authority) &&
        wasi_http_types_method_outgoing_request_body(wasi_http_types_borrow_outgoing_request(req), &out_body);
    ...
    if (body != NULL) {
        ...
        if (!wasi_http_types_method_outgoing_body_write(wasi_http_types_borrow_outgoing_body(out_body), &ret)) {
            printf("Error getting output stream\n");
            return 7;
        }
        client_list_u8_t buf = {
            .ptr = (uint8_t *) body,
            .len = strlen(body),
        };
        wasi_io_streams_stream_error_t stream_err;
        // TODO check error here.
        wasi_io_streams_method_output_stream_blocking_write_and_flush(wasi_io_streams_borrow_output_stream(ret), &buf, &stream_err);
        wasi_io_streams_output_stream_drop_own(ret);
    }

    wasi_http_outgoing_handler_own_future_incoming_response_t ret;
    wasi_http_outgoing_handler_error_code_t handler_err;
    if (!wasi_http_outgoing_handler_handle(req, NULL, &ret, &handler_err)) {
        printf("Error sending request\n");
        return 5;
    }

    wasi_http_types_error_code_t finish_err;
    if (!wasi_http_types_static_outgoing_body_finish(out_body, NULL, &finish_err)) {
        printf("Failed to finish body\n");
        return 10;
    }

    wasi_http_types_own_pollable_t poll = wasi_http_types_method_future_incoming_response_subscribe(wasi_http_types_borrow_future_incoming_response(ret));
    wasi_io_poll_method_pollable_block(wasi_io_poll_borrow_pollable(poll));

    wasi_http_types_result_result_own_incoming_response_error_code_void_t result;
    if (!wasi_http_types_method_future_incoming_response_get(wasi_http_types_borrow_future_incoming_response(ret), &result)) {
    ...
    wasi_http_types_future_incoming_response_drop_own(ret);

    wasi_http_types_own_incoming_response_t resp = result.val.ok.val.ok;
    wasi_http_types_status_code_t code = wasi_http_types_method_incoming_response_status(wasi_http_types_borrow_incoming_response(resp));
    ...
    wasi_http_types_fields_drop_own(header_handle);

    wasi_http_types_own_incoming_body_t in_body;
    if (!wasi_http_types_method_incoming_response_consume(wasi_http_types_borrow_incoming_response(resp), &in_body)) {
        printf("body is error!\n");
        return 3;
    }
    wasi_io_streams_own_input_stream_t stream;
    if (!wasi_http_types_method_incoming_body_stream(wasi_http_types_borrow_incoming_body(in_body), &stream)) {
        printf("stream is error\n");
        return 9;
    }

    wasi_io_streams_own_pollable_t stream_poll = wasi_io_streams_method_input_stream_subscribe(wasi_io_streams_borrow_input_stream(stream));

    int32_t offset = 0;
    while (offset < response_out->body_max_len) {
        wasi_io_poll_method_pollable_block(wasi_io_poll_borrow_pollable(stream_poll));
        client_list_u8_t body_res;
        wasi_io_streams_stream_error_t stream_err;
        if (wasi_io_streams_method_input_stream_read(wasi_io_streams_borrow_input_stream(stream), response_out->body_max_len - offset, &body_res, &stream_err)) {
            ...
            client_list_u8_free(&body_res);
        } else if (stream_err.tag == WASI_IO_STREAMS_STREAM_ERROR_CLOSED) {
            break;
        } else {
    ...
    // wasi_http_types_outgoing_request_drop_own(req);
